...
class PortfolioSummary:
    ...
    net_yield_pct:                  float
    portfolio_appreciation_pct_4q:  float   # weighted NHB Residex quarterly forecast
    avg_risk_score:                 float
    diversification_score:          float
    locality_concentration:         dict    # locality → % of portfolio value
...
class M4PortfolioAnalyser:
    """Rule-based portfolio aggregation and advisory model (M4).

    Works with any DataFrame that contains per-property columns from M1–M6.
    Uses 'locality' (city / sub-market) as the geographic grouping key
    instead of the US-style zip code.
    """
    ...
    def analyse(
        ...
        appr_col  = self._pick(df, ["m3_appreciation_pct_quarterly",
                                     "m3_appreciation_pct_1q",
                                     "appreciation_pct_4q"],                       default=5.0)
        ...
        # Weighted average appreciation (value-weighted)
        wtd_appr = float(
            (df[val_col] * df[appr_col]).sum() / (total_value + 1e-9)
        )

        # ── Geographic concentration  (locality / city) ──────────────────────
        locality_col = None
        for candidate in ["locality", "CITY", "zip_code"]:
            if candidate in df.columns:
                locality_col = candidate
                break

        if locality_col:
            loc_conc = (
                df.groupby(locality_col)[val_col].sum() / total_value * 100
            ).round(1).to_dict()
        else:
            loc_conc = {}

        # ── Diversification score ────────────────────────────────────────────
        div_score = self._diversification_score(df, val_col, risk_col, tier_col, locality_col)
        ...
        rank_cols = [c for c in [
            "property_id", locality_col,
            val_col, rent_col, maint_col, appr_col, risk_col,
            "_net_yield_prop",
        ] if c is not None and c in df_s.columns]
        ...
        flags = self._rebalancing_flags(
            df, loc_conc, net_yield, gross_yield,
            df[risk_col].values, tier_col,
        )

        suggestion = self._next_investment_suggestion(df, loc_conc, net_yield)
        ...
        summary = PortfolioSummary(
            ...
            portfolio_appreciation_pct_4q=round(wtd_appr, 2),
            avg_risk_score=round(avg_risk, 1),
            diversification_score=round(div_score, 1),
            locality_concentration=loc_conc,
    ...
    def _diversification_score(
        ...
        locality_col: Optional[str],
    ) -> float:
        scores = []

        if locality_col and locality_col in df.columns and len(df) > 1:
            total = df[val_col].sum()
            if total > 0:
                shares = df.groupby(locality_col)[val_col].sum() / total
                hhi    = float((shares ** 2).sum())
                scores.append((1.0 - hhi) * 100)
            n_localities = df[locality_col].nunique()
            scores.append(min(n_localities / max(len(df) * 0.5, 1), 1.0) * 100)
    ...
    def _rebalancing_flags(
        ...
        for loc, pct in loc_conc.items():
            if pct > 50:
                flags.append(
                    f"Geographic concentration: {pct:.0f}% of portfolio value in "
                    f"'{loc}'. Consider diversifying into a different Mumbai sub-market."
                )
        ...
        if avg_risk > 65:
            flags.append(
                f"Portfolio average risk score {avg_risk:.0f}/100 exceeds 65 — "
                f"reduce exposure to high-volatility localities."
            )
    ...
    def _next_investment_suggestion(
        ...
        n_localities = len(loc_conc)
        if n_localities < 2:
            return (
                "Portfolio is concentrated in a single locality. "
                "Next acquisition should target a geographically distinct Mumbai "
                "sub-market with strong NHB Residex HPI growth (above 5% per year)."
            )
        if n_localities < 4:
            return (
                f"With {n_localities} localities represented, diversification is partial. "
                "Target a locality with low current portfolio exposure, "
                "strong employment base, and improving rental demand."
            )
        if net_yield < 3.5:
            return (
        ...
        return (
            "Portfolio is broadly diversified across Mumbai sub-markets. "
            "Seek the next acquisition in a locality with NHB Residex HPI "
            "appreciation forecast > 7% (annualised) and gross yield > 5%, "
    ...
    def to_llm_context(summary: PortfolioSummary) -> dict:
        return {
            ...
            "appreciation_forecast_4q":    f"{summary.portfolio_appreciation_pct_4q:.1f}%",
            "avg_risk_score":              summary.avg_risk_score,
            "diversification_score":       summary.diversification_score,
            "locality_concentration":      summary.locality_concentration,
